...
const ArtistSessions: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const countSessionsByArtist = async (artist: Artist) => {
      ...
      const sessionsInSelectedDate = sessions.filter(
        (session) =>
      ...
      return sessionsInSelectedDate.length;
    };

    const fetchData = async () => {
      const loadedArtists = await loadArtists();
      const sessionsByArtistPromises = loadedArtists.map(async (artist) => ({
        name: `${artist.name}: ${await countSessionsByArtist(artist)}`,
        data: await countSessionsByArtist(artist),
      }));

      Promise.all(sessionsByArtistPromises).then((sessionsByArtist) => {
        setChartData(sessionsByArtist);
        const hasData = sessionsByArtist.some((item) => item.data !== 0);
        if (!hasData) {
